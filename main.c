...
int main(void)
{
	...
	Init_USART_GPIO_DS18B20();
		...
		if(contr<50){contr=50;} write_seatings();//контраст по умолчанию
		Set_contr(contr);
...
//***************************************************************************************************************************************//
   ...
   if(cstart==0){clock();} //	рисуем  время и календарь
   if(cstart==1){clock_1();}
   if(cstart==2){clock_2();}
   if(cstart==3){clock_3();}
...
//********************кнопочки*******************************//
    	  ...
    	  if((keys==1||keys==3)&!S_dat){cstart++; if(cstart>3){cstart=0;}lcd_clear();}

    	       if	(keys==2) { S_dat++; }//менюшка установки время календарь, однократное нажатие
    	       if	(keys==20) {  list++; S_dat=0; lcd_clear(); delay_ms(300);}  //менюшка установки системные настройки длительное нажатие
     ...
     if(list==1)
    	           {
    	       	  ...
    	       	  {
    	         	  	  	  //L=(84-7*6)/2;
    	           	       lcd_set_strs(1,18,8,"КОНТРАСТ",0);
    	          	       	   	  // L=(84-9*6)/2;
    	           	       //lcd_set_strs(1,15,8,"ПОДСВЕТКИ",0);

    	           	       	if	(keys==1){contr++; if(contr>126){contr=0;} Set_contr(contr);}
    	           	       	if	(keys==3){contr--; if(contr>126){contr=126;} Set_contr(contr);}
...
//*********************поисk датчика 3************************************************************************//
   ...
   if(list==7){BipStop(); lcd_set_strs(2,24,8,"ГОТОВО",0);
...
//********************************************************************************************************************//
 void clock_1()
 {
	 ...
	 if(S_dat==2) {lcd_set_strs(0,2,24,buf,1);if(keys==1) {timer+=3600; }
	  	  	  	  	  	  	  	  	  	  	  	 if(keys==3) {timer-=3600; }
	  	  	  	  	  	  	  	  	  	  	  	  	RTC_SetCounter_(timer); }
	 	 	 else  lcd_set_strs(0,2,24,buf,0);

	    	 _sprtffd(0,buf,unix_time.min);
	    	 conv_dir (2,'0');

	  if(S_dat==1) {lcd_set_strs(0,50,24,buf,1);if(keys==1) {timer+=60; }
	    	 	  	  	  	  	  	  	  	  	 if(keys==3) {timer-=60; }
	    	 	  	  	  	  	  	  	  	  	  	 RTC_SetCounter_(timer); }
	  	  	  else  lcd_set_strs(0,50,24,buf,0);


	  	  	  lcd_set_strs(3,5,16,"OUT",0);
	  	  	  T_out(3,43,16,0);

	    	   lcd_set_strs(5,0,8,"D",0);
	    	   T_eng(5,7,8,0);
	    	   ...
	    	   if(!S_dat){__WFI();}//если не в меню то спать
	    	     	  // __WFI();
	    	     	 //**спаль до прерывания по таймеру сек или  индикации
	    	     	 if(S){lcd_set_strs(0,34,24,":", 0);}//рисуем точки и спать
	    	     	 	 else
	    	     	       	   {
	    	     	 		 	 lcd_set_rect(0,40,4,1);//убираем точки и спать
	    	 				    lcd_set_rect(1,40,4,1);
	    	 				    lcd_set_rect(2,40,5,1);
	    	 				   if(!S_dat){__WFI();}//если не в меню то спать
	    	 				   // __WFI();
	    	 				    }
	    	     	if(S_dat>2){ S_dat = 0;BipStop();}
 ...
 //************************выгружаем  время*******************************************************************************//
  void clock_2()
  {
 	 ...
 	 if(S_dat==3) {lcd_set_strs(0,10,16,buf,1);if(keys==1) {timer+=3600; }
 	  	  	  	  	  	  	  	  	  	  	  	 if(keys==3) {timer-=3600; }
 	  	  	  	  	  	  	  	  	  	  	  	  	RTC_SetCounter_(timer); }
 	 	 	 else  {lcd_set_strs(0,10,16,buf,0);}
 	 	 	 	 	 lcd_set_strs(0,26,16,":",0);

 	    	 _sprtffd(0,buf,unix_time.min);
 	    	 conv_dir (2,'0');

 	  if(S_dat==2) {lcd_set_strs(0,34,16,buf,1);if(keys==1) {timer+=60; }
 	    	 	  	  	  	  	  	  	  	  	 if(keys==3) {timer-=60; }
 	    	 	  	  	  	  	  	  	  	  	  	 RTC_SetCounter_(timer); }
 	  	  	  else { lcd_set_strs(0,34,16,buf,0);}

 	  	  	  lcd_set_strs(0,50,16,":",0);

 	  	  	_sprtffd(0,buf,unix_time.sec);
 	  	  	 	    	 conv_dir (2,'0');

 	  	  	 	  if(S_dat==1) {lcd_set_strs(0,58,16,buf,1);if(keys==1) {timer+=1; }
 	  	  	 	    	 	  	  	  	  	  	  	  	  	 if(keys==3) {timer-=1; }
 	  	  	 	    	 	  	  	  	  	  	  	  	  	  	 RTC_SetCounter_(timer); }
 	  	  	 	  	  	  else  {lcd_set_strs(0,58,16,buf,0);}

 	  	  	 lcd_set_strs(2,0,16,"OUT",0);
 	  	  	 T_out(2,40,16,0);
 	  	  	 lcd_set_strs(4,0,16,"IN",0);
 	  	  	 T_in(4,40,16,0);
 	  	  	 //Bat(6,0,8,0);//напряжение сети

 	  	  if(!S_dat){__WFI();}//если не в меню то спать
 	  	if(S_dat>3){ S_dat = 0;BipStop();}
  	 ...
  	 if(S_dat==3) {lcd_set_strs(0,10,16,buf,1);if(keys==1) {timer+=3600; }
  	  ...
  	  if(S_dat==2) {lcd_set_strs(0,34,16,buf,1);if(keys==1) {timer+=60; }
  	  	  	...
  	  	  	_sprtffd(0,buf,unix_time.sec);
  	  	  	 ...
  	  	  	 lcd_set_strs(2,0,16,"ENG",0);
  	  	  	 T_eng(2,40,16,0);
  	  	  	 lcd_set_strs(4,0,16,"BAT",0);
  	  	  	 Bat(4,40,16,0);
  	  	  	 //Bat(6,0,8,0);//напряжение сети
...
{u8 l;
   ...
   } else {lcd_set_strs(y,l,h,"----",0);}//датчик не установлен
}
//*********************************************************************************************************************//
//******************температура в******************************//
void T_in(u8 y,u8 x, u8 h,u8 c)
  ...
  } else {lcd_set_strs(y,l,h,"----",c);}//датчик не установлен
  }
//*********************************************************************************************************************//
//*****************температура двиг***************************//
...
//**********************подключение датчиков**************************************************************************//
 ...
 void set_d_ds18b20 (unsigned char set_d[16])//set_d ПИШЕМ ЗАГАЛОВОК
  {int i;
     		...
     		else {lcd_set_rect(3,0,84*2,1); lcd_set_strs(3,22,16,"ERROR",1);
     		delay_ms(400); }
